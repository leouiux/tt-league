...
function processBulk() {
  ...
  const players = text.split('\n').map(l => l.trim()).filter(l => l).map(line => {
    ...
    if (tokens.length >= 3) {
      ...
      const last = tokens[1];
      if (/\d/.test(last)) { rawGrade = last; rawName = tokens[0]; }
      else { rawName = tokens[0]; rawAffil = tokens[1]; }
    } else {
      rawName = tokens[0] || '';
    }
    ...
    const grade = rawGrade ? cleanGrade(rawGrade) : 9999;
...
h1 { text-align: center; font-size: ${titleFontPt}pt; font-weight: 900; margin-bottom: 10px; flex-shrink: 0; letter-spacing: -0.3px; }
...
thead tr { height: 28px; }
th { background: #e8eaf0; border: 1.5px solid #000; font-size: ${cellFontPt}pt; font-weight: 800; text-align: center; vertical-align: middle; }
...
*{box-sizing:border-box;margin:0;padding:0;}
html,body{width:100%;height:100%;font-family:'Malgun Gothic',sans-serif;}
body{padding:3mm;height:100vh;display:flex;flex-direction:column;}
.page{flex:1;display:flex;flex-direction:column;height:100%;}
h2{text-align:center;font-size:${Math.min(18,Math.max(12,cellFontPt+4))}pt;font-weight:900;margin-bottom:4px;flex-shrink:0;display:flex;align-items:baseline;justify-content:center;gap:12px;}
.print-hint{font-size:9pt;font-weight:700;color:#dc2626;white-space:nowrap;}
.table-wrap{flex:1;display:flex;flex-direction:column;min-height:0;}
table{width:100%;border-collapse:collapse;flex:1;table-layout:fixed;}
th,td{border:2px solid #000;padding:0 4px;text-align:center;font-size:${cellFontPt}pt;font-weight:bold;vertical-align:middle;}
th{background:#f0f0f0;}
tbody tr{height:calc((100vh - 26mm) / ${n + 1});}
.pn{text-align:left;padding-left:8px;word-break:keep-all;}
.sched{flex-shrink:0;margin-top:4px;}
.sched-matches{text-align:left;line-height:1.7;font-size:7pt;font-weight:600;}
.sched-item{display:inline-block;white-space:nowrap;margin-right:6px;}
.pbtn{display:block;margin:6px auto 0;padding:7px 24px;font-size:11pt;background:#5b6cf5;color:white;border:none;border-radius:8px;cursor:pointer;font-weight:bold;}
@media print{.pbtn{display:none;}.print-hint{display:none;}}
</style></head><body>
<div class="page">
  <h2>${league.title} — ${gn} 대진표 <span class="print-hint">인쇄 시 머리글과 바닥글 체크해제 / 배경그래픽 체크 꼭 해주세요!</span></h2>
  <div class="table-wrap">
    <table>
  ...
  <div class="sched">
    <div class="sched-matches">${schedule.map((s,i) => `<span class="sched-item">${s}${i < schedule.length-1 ? ' /' : ''}</span>`).join(' ')}</div>
  </div>
  <button class="pbtn" onclick="window.print()">🖨️ 인쇄</button>
</div></body></html>`);
  w.document.close();
