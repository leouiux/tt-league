...
window.printAllPlayers = function() {
  ...
  // margin: top/left/right 15mm, bottom 5mm → 인쇄 가용: 267mm × 190mm
  // 제목 약 22pt(~7.8mm) + mb 3pt → 표 가용 높이 ≈ 190mm - 8.5mm = 181.5mm ≈ 514pt
  // thead 20pt → tbody 가용 = 514 - 20 = 494pt
  const theadPt = 20;
  const rowHPt  = Math.floor(494 / rows);

  w.document.write(`<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8">
<title>${league.title} 참가자명단</title>
<style>
@page { size: A4 landscape; margin: 15mm 15mm 5mm 15mm; }
* { box-sizing: border-box; margin: 0; padding: 0; }
html, body { font-family: 'Malgun Gothic', sans-serif; background: #fff; }
h1 { text-align: center; font-size: ${Math.min(titleFontPt, 16)}pt; font-weight: 900; margin-bottom: 3pt; letter-spacing: -0.3px; line-height: 1.1; }
...
td { border: 1px solid #000; font-size: ${cellFontPt}pt; vertical-align: middle; padding: 0 2pt; }
...
window.printGroupMatrix = function(gn) {
  ...
  // A4 landscape: 297mm × 210mm
  // margin: top/left/right 6mm, bottom 5mm → 인쇄 가용: 285mm × 199mm
  // h2 약 20pt + mb 2pt, 경기순서 약 18pt, 여백 합계 ≈ 42pt
  // 테이블 가용 높이 ≈ 199mm×(72/25.4) - 42 ≈ 564 - 42 = 522pt
  const tableAvailPt = 522;
  const rowHPt = Math.floor(tableAvailPt / (n + 1));

  w.document.write(`<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8">
<title>${league.title} - ${gn}</title>
<style>
@page{size:A4 landscape;margin:6mm 6mm 5mm 6mm;}
*{box-sizing:border-box;margin:0;padding:0;}
html,body{font-family:'Malgun Gothic',sans-serif;background:#fff;}
h2{text-align:center;font-size:${Math.min(17,Math.max(11,cellFontPt+3))}pt;font-weight:900;margin-bottom:2pt;display:flex;align-items:baseline;justify-content:center;gap:8pt;line-height:1.1;}
.print-hint{font-size:7pt;font-weight:700;color:#dc2626;white-space:nowrap;}
table{width:100%;border-collapse:collapse;table-layout:fixed;}
th,td{border:1px solid #000;padding:0 2pt;text-align:center;font-size:${cellFontPt}pt;font-weight:bold;vertical-align:middle;height:${rowHPt}pt;}
th{background:#f0f0f0;}
.pn{text-align:left;padding-left:4pt;word-break:keep-all;}
.sched{margin-top:2pt;}
