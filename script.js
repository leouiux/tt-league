...
.sched-matches{text-align:left;line-height:2;font-size:6pt;font-weight:600;}
.sched-item{display:inline-block;white-space:nowrap;}
.sched-sep{display:inline-block;margin:0 4pt;}
.pbtn{display:block;margin:10px auto;padding:8px 28px;font-size:11pt;background:#5b6cf5;color:white;border:none;border-radius:8px;cursor:pointer;font-weight:bold;}
@media print{
  .pbtn{display:none;}
...
<h2>${league.title} — ${gn} 대진표 <span class="print-hint">인쇄 시 머리글/바닥글 체크해제, 배경그래픽 체크 필수!</span></h2>
  ...
  <div class="sched">
    <div class="sched-matches">${schedule.map((s,i) => `<span class="sched-item">${s}</span>${i < schedule.length-1 ? '<span class="sched-sep">/</span>' : ''}`).join('')}</div>
  </div>
  <button class="pbtn" onclick="window.print()">🖨️ 인쇄</button>
</body></html>`);
  w.document.close();
